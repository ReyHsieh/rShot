...
### 标注（iShot 式原位编辑）
框选松手后**不弹新窗口**——截图留在屏幕原位、四周暗色遮罩，工具栏浮岛出现在选区正下方（水平也锚定选区，空间不足时自动上移/居中）：

- **矩形框选**、**箭头**（实心三角头）、**高亮笔**
- **文本**：底框填充 / 透明双模式，**多行编辑**（Enter 换行、失焦提交、双击重新编辑）；底框随输入自适应宽高，**中文拼音输入时实时横向展开**（原生 NSTextView，不打断输入法），文本垂直居中
- **数字标记**：①②③ 自动递增，分步说明利器
- **马赛克**：真·像素化（区域缩小 + 无插值放大）
- **画笔**：自由轨迹
- 拖动移动任意标注（箭头/画笔轨迹整体平移）、右键删除、⌘Z/⌘⇧Z 撤销重做
- 候选条紧贴工具栏上方展开（输入法候选词式），每工具独立记忆上次设置
- 完成操作区（保存/取消/复制）为独立图标浮岛，与工具栏分离
- **Enter 完成截图**（文本编辑中除外）、ESC 取消
...
- **v1.2**：Enter 完成截图；文本多行编辑（Enter 换行/失焦提交）；底框随输入自适应宽高（拼音实时横向展开、垂直居中）；浮岛水平锚定选区；菜单栏菜单改系统标准弹出（修复首帧不完整）
- **v1.1**：开机自启动；Dock 图标开关；菜单栏设置同步修复；OCR 卡顿修复；操作浮岛拆分；设置窗口自适应
- **v1.0**：首个完整版本（截图/标注/OCR/快捷键/设置，21 项单元测试）
